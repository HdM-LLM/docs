...
[![The classdiagram containing all the classes](./images/classdiagramm.png)](./images/classdiagramm.png)
...
* The creation of categories and their chips in function `assign_chip` inside [category_service.py](https://github.com/HdM-LLM/backend/blob/main/src/services/category_service.py)
...
The services in general are not only used for API interactions with OpenAI but also for other operations, with different classes. For this reason each class has his own service, which is responsible for the operations of the class.

We used multiple different python packages to implement the backend. You can find all packages [here](https://github.com/HdM-LLM/backend/blob/main/requirements.txt) inside the requirements.txt.

All functions inside the the backend repository is documented with the help of the [autoDocstring extension](https://marketplace.visualstudio.com/items?itemName=njpwerner.autodocstring) for Visual Studio Code. This is done to make sure, that the code is easy to understand and to maintain.
